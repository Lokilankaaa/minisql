...
void RecordManager::searchBlockWithIndex(std::string table_name, std::string target_attr, Constraint target_cons, std::vector<int> &allBlockId, std::string index_name){
...
void RecordManager::createIndex(IndexManager &index_manager, std::string table_name, std::string target_attr, std::string index_name){
    ...
    if(count_block <= 0) count_block=1;
    //获取对应表的索引文件
    std::string indexFilePath = "INDEX_FILE_"+target_attr+"_"+table_name+"_"+index_name;
    for(int i=0; i<count_block; i++){
        char *p = buf_manager.getPage(temp_path, i);
        char *temp = p;
        while(true){
    ...
    if(judge==true){
        ...
        for(int i=0; i<indexindex.size(); i++){
            ...
            if(indexindex[i]!=-1 && target_cons[i].conSymbol != NOT_EQUAL){
                ...
                searchBlockWithIndex(table_name, target_attr[i], target_cons[i], block_ids, allIndex.name[indexindex[i]]);
