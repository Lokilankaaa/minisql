...
void RecordManager::searchBlockWithIndex(std::string table_name, std::string target_attr, Constraint target_cons,
                                         std::vector<int> &allBlockId, std::string index_name) {
    IndexManager im(table_name);
    data temp_data;
    std::string temp_path = "INDEX_FILE_" + target_attr + "_" + table_name + "_" + index_name;
...
void
RecordManager::searchRecordAccordCons(std::string table_name, int block_id, attributes_set target_attr, int attr_index,
                                      Constraint target_cons, std::vector<TUPLE> &result) {
    ...
    while (true) {
        ...
        if (target_attr.type[attr_index] == -1) {
            //int
            if (satisfyWithCondition(single_data[attr_index].int_data, target_cons.conData.int_data, target_cons) ==
                true) {
                result.push_back(tuple);
            }
        } else if (target_attr.type[attr_index] == 0) {
            //float
            if (satisfyWithCondition(single_data[attr_index].float_data, target_cons.conData.float_data, target_cons) ==
                true) {
                result.push_back(tuple);
            }
        } else {
            //char
            if (satisfyWithCondition(single_data[attr_index].char_data, target_cons.conData.char_data, target_cons) ==
                true) {
                result.push_back(tuple);
            }
        }
...
//判断要插入的记录是否和其他记录有冲突
bool
RecordManager::judgeConflict(const std::vector<TUPLE> &table_tuples, std::vector<data> &target_value, int attr_index) {
...
int RecordManager::deleteRecordAccordCons(std::string table_name, int block_id, attributes_set target_attr,
                                          std::vector<int> attr_index, std::vector<Constraint> target_cons) {
    ...
    while (true) {
        ...
        for (int i = 0; i < attr_index.size(); i++) {
            ...
            if (target_attr.type[attr_index[i]] == -1) {
                //int
                if (satisfyWithCondition(allData[attr_index[i]].int_data, target_cons[i].conData.int_data,
                                         target_cons[i]) == false) {
                    judge = false;
                }
            } else if (target_attr.type[attr_index[i]] == 0) {
                //float
                if (satisfyWithCondition(allData[attr_index[i]].float_data, target_cons[i].conData.float_data,
                                         target_cons[i]) == false) {
                    judge = false;
                }
            } else {
                //char
                if (satisfyWithCondition(allData[attr_index[i]].char_data, target_cons[i].conData.char_data,
                                         target_cons[i]) == false) {
                    judge = false;
                }
            }
...
table RecordManager::selectRecord(std::string table_name, std::string target_attr, Constraint target_cons,
    ...
    for (int i = 0; i < allAttr.num; i++) {
        if (allAttr.name[i] == target_attr) {
            ...
            index = i;
            //查找是否存在索引
            for (int j = 0; j < allIndex.num; j++) {
                if (allIndex.location[j] == index) {
                    indexindex = j;
                    break;
                }
            }
    ...
    if (indexindex != -1 && target_cons.conSymbol != NOT_EQUAL) {
        std::vector<int> all_Blocks;
        //使用索引查找
        searchBlockWithIndex(table_name, target_attr, target_cons, all_Blocks, allIndex.name[indexindex]);
    ...
    for (int i = 0; i < allAttr.num; i++) {
        for (int j = 0; j < allIndex.num; j++) {
            if (i == allIndex.location[j]) {
                ...
                std::string thePath = "INDEX_FILE_" + theName + "_" + table_name + "_" + allIndex.name[j];
            ...
            for (int j = 0; j < allAttr.num; j++) {
                for (int k = 0; k < allIndex.num; k++) {
                    if (allIndex.location[k] == j) {
                        //如果有索引的话
                        std::string theName = allAttr.name[i];
                        std::string thePath = "INDEX_FILE_" + theName + "_" + table_name + "_" + allIndex.name[k];
                        std::vector<data> allData = tuple.getData();
...
void RecordManager::createIndex(IndexManager &index_manager, std::string table_name, std::string target_attr,
                                std::string index_name, int type) {
    std::string temp_path = DATA_FILE_PATH + table_name;
    catalog_manager cate;
    attributes_set allAttr = cate.getAllattrs(table_name);
    //用于标记目标属性的下标
    ...
    if (count_block <= 0) count_block = 1;
    //获取对应表的索引文件
    std::string indexFilePath = "INDEX_FILE_" + target_attr + "_" + table_name + "_" + index_name + "_" + num2str(type) + ".txt";
    for (int i = 0; i < count_block; i++) {
        char *p = buf_manager.getPage(temp_path, i);
        char *temp = p;
        while (true) {
...
//输入：(表名,属性集合,约束集合)
//输出：删除数据的数量
int RecordManager::deleteRecord(std::string table_name, std::vector<std::string> target_attr,
